 ...
 *     modal Acción ✈ (si no, el candado se saltaba en dos clics);
 *     los demás campos —programa, promo, agenda, datos— se siguen
 *     editando igual. Al guardar se conserva el estado que ya
 *     tenía, así ningún otro campo puede bajarlo de estado sin que
 *     nadie lo pida. El DESARROLLADOR queda fuera del candado.
 *   · Ofertas de Empleo: botón "🎟️ Liberar cupo" en el panel de
 *     Participantes. Aparece solo cuando la entrevista terminó en
 *     NO APROBADA y ese cupo quedó ocupado porque la oferta tenía
 *     la política "el cupo se pierde". Al liberarlo, la oferta
 *     vuelve a sumar ese cupo entre los disponibles. Va EN
 *     SILENCIO: no se envía WhatsApp ni correo a nadie, y el
 *     participante sigue como No aprobado. Queda escrito en el
 *     historial de la oferta, en el de la aplicación y en
 *     Auditoría. El cupo no se puede devolver dos veces.
 *   Archivos tocados: app.js, js/ofertas.js, sw.js (caché v29) y,
 *   en Apps Script, OfertasProceso.gs y Código.gs.
 * ------------------------------------------------------------
 * CHIP DEL USUARIO — ARREGLO (05/09/2026).
 *   · El destello faltaba justo en la barra de Bienvenida del Inicio:
 *     la foto de esa barra lleva z-index propio y, al pasar a ser un
 *     ítem flexible, se pintaba por encima del brillo. Ya se ve en las
 *     cinco vistas por igual.
 *   Archivo tocado: styles.css.
 * ------------------------------------------------------------
 * CHIP DEL USUARIO EN LAS CABECERAS (05/09/2026).
 *   · La foto del usuario ya no es una imagen quieta: lleva un DESTELLO
 *     en vivo (un brillo que la cruza) en la barra de Bienvenida, en las
 *     tarjetas de Usuarios y en el chip nuevo. Si el sistema del equipo
 *     pide menos movimiento, el destello no se anima.
 *   · Al tocar cualquiera de esas fotos se abre el MISMO visor con zoom
 *     de Nivel de Inglés (rueda, pellizco, arrastre y doble clic).
 *   · En las 11 vistas con cabecera aparece arriba a la derecha el
 *     usuario en sesión: foto, nombre y rol con el color del sistema.
 *     En teléfono queda solo la foto y al tocarla se abre una tarjetita
 *     con el nombre, los roles y el botón para ver la foto en grande.
 *   · En teléfono el título de la cabecera cede el ancho (se recorta con
 *     puntos suspensivos) para que el chip quepa junto a los iconos.
 *   Archivos tocados: index.html, app.js, styles.css, css/tema-oscuro.css,
 *   js/nivel.js, sw.js (caché v28).
 * ------------------------------------------------------------
 * LOTE MIS BOTS · PARTE 2 (05/09/2026) — CADA VISTA MANDA POR SU BOT.
 *   · Los avisos de WhatsApp del Contador, de Nivel de Inglés y de las
 *     Ofertas de empleo ya salen por el NÚMERO DE PROCESOS. Antes solo
 *     lo hacían los de ofertas: todo lo demás salía por el comercial.
 *   · El bot ya no lo decide la vista sino la PLANTILLA: en
 *     Configuración → Plantillas cada una lleva su área (Comercial ·
 *     Procesos · General) y se ve de un vistazo, con pastilla de color,
 *     por cuál bot sale. Se cambia ahí mismo, sin tocar código, y
 *     arriba hay un resumen de cuántas van por cada bot.
 *   · Lo que quede sin clasificar sigue saliendo por el Bot Comercial,
 *     igual que hoy: nada se cae por un dato que falte.
 *   Archivos tocados: index.html, app.js, styles.css, sw.js.
 * ------------------------------------------------------------
 * FASE 4.3 (05/09/2026) — EL RITMO VISUAL DEL PDF DE OFERTAS.
 *   (incluye los dos ajustes finales del 05/09: aire entre el título
 *   y su texto en la caja PROGRAM DATES / JOB CONDITIONS de la página
 *   1, y los datos cortos —MEALS INCLUDED, HOUSING COST…— con el
 *   mismo tamaño y el mismo aire que los demás subtítulos.)
 *   Solo toca el PDF; ni el portal ni la Zona de estudiantes cambian.
 *   · El aire entre título, subtítulo y texto ya no se hace con
 *     renglones en blanco sino con espaciado de párrafo de verdad, con
 *     una sola tabla de medidas para todo el documento: nada queda
 *     pegado en un lado y suelto en otro.
 *   · Los subtítulos resaltan: antes iban en letra MÁS PEQUEÑA que el
 *     cuerpo (por eso no se veían); ahora van más grandes, en negrilla
 *     y del azul de los títulos. El texto general sigue sin negrilla.
 *   · Los Enter y espacios de más que se escriban en los campos se
 *     siguen ignorando a propósito: el espaciado lo pone el sistema,
 *     no hay que empujarlo a mano desde la configuración de la oferta.
 *   · Los saltos de página se calculan midiendo en PUNTOS lo que de
 *     verdad ocupa cada bloque, no contando renglones: la página se
 *     llena y solo se pasa a la siguiente cuando ya no cabe. Se acabó
 *     la media página vacía. Lo único que sigue bajando de página es
 *     un título que quedaría solo al pie.
 *   Archivo tocado: Apps Script OfertasPdf.gs.
 * ------------------------------------------------------------
 * FASE 4.2 (04/09/2026) — AJUSTES DE OFERTAS Y DEL PDF.
 *   · Horas por semana y Temperatura promedio ya se escriben LIBRES:
 *     además de "32" se puede poner "32–40" o "20–28 °C". El sufijo
 *     (hours/week, °C) lo pone el sistema solo si el texto no lo trae
 *     ya, así que nunca queda duplicado.
 *   · Employer description tiene tope de 270 caracteres, con contador
 *     en vivo bajo el campo (gris mientras haya margen, rojo al
 *     pasarse) y el botón Guardar bloqueado hasta recortarlo. Con ese
 *     tope la descripción siempre cabe en la página 1 del PDF.
 *   · PDF de la oferta: aire entre cada subtítulo y su texto; el
 *     sistema ya NO pone viñetas —salen las que escribe SEP y solo
 *     esas—; la etiqueta de los datos cortos ("Meals included",
 *     "Housing cost"…) va en negrilla; el nombre del empleador ya no
 *     se monta sobre las fotos de la cabecera (la caja se recorta y
 *     la letra se encoge hasta que cabe, sin cortar el nombre); y
 *     ENGLISH LEVEL sale en inglés, que era lo único que quedaba en
 *     español dentro del documento.
 *   · Cuando la selección de una oferta con Sponsor INTRAX queda
 *     confirmada, al participante le llega un correo adicional con la
 *     aplicación de INTRAX: obligatoria, se la envían en 2 días
 *     hábiles, tiene 5 días calendario para devolverla y sin ella no
 *     se asegura su cupo. La plantilla se edita en Configuración →
 *     Plantillas (OFERTA_INTRAX) y no se manda dos veces.
