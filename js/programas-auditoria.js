...
(function () {
  ...
  /* app.js declara `let currentUser`: con `let` NO hay window.currentUser.
     Se lee como identificador del ámbito global. */
  function uid_() {
    try { if (typeof currentUser !== 'undefined' && currentUser) return currentUser.id || ''; } catch (e) { /* aún no existe */ }
  ...
  function pintar_() {
    ...
    A.cargando = true;
    var mio = ++A.pedido;
    apiGet('auditarProgramas', { usuarioId: uid_() }, { silent: true })
      .then(function (d) {
        A.cargando = false;
