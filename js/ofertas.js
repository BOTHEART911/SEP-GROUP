  ...
  return `<article class="com-card ofe-card" data-ofe-id="${esc_(o.id)}">
    ...
    <div class="ofe-card__body">
      ...
      <div class="ofe-card__datos">
        ...
        <span>${esc_(o.horasTexto || OFE_SIN_DATO)}</span>
      ...
      <div class="ofe-card__acciones">
        ...
        <button class="btn btn-ghost" data-ofe-pdf="${esc_(o.id)}">📄 PDF</button>
...
function renderOfeCards_() {
  ...
  cont.querySelectorAll('[data-ofe-pdf]').forEach(b =>
    b.addEventListener('click', () => ofePdf_(b.getAttribute('data-ofe-pdf'), false)));
}

/* ============================================================
   PDF DE LA OFERTA — Entrega 4 (1.32)
   ============================================================
   El PDF no se arma aquí: lo hace el backend con la plantilla de
   Configuración. Esta pantalla solo lo pide, lo abre y —si quien
   mira es SUPER o DEV— deja rehacerlo cuando la oferta cambió y
...
async function ofePdf_(id, forzar) {
  ...
  try {
    ...
    const r = await Swal.fire({
      icon: 'success',
      title: d.reusado ? 'PDF listo' : (d.regenerado ? 'PDF actualizado' : 'PDF generado'),
      html: `<div style="text-align:left">
        <p><b>${esc_(d.nombre || '')}</b><br><span class="muted">${d.paginas || 0} página(s)</span></p>
        ${avisos.length ? '<ul style="margin:8px 0 0 16px">' + avisos.map(a => `<li>${esc_(a)}</li>`).join('') + '</ul>' : ''}
      </div>`,
      showConfirmButton: true, confirmButtonText: 'Abrir',
      showDenyButton: true, denyButtonText: 'Copiar enlace',
      showCancelButton: admin, cancelButtonText: 'Rehacer'
    });
    if (r.isConfirmed) window.open(d.url, '_blank', 'noopener');
    else if (r.isDenied) {
      try {
        await navigator.clipboard.writeText(d.url);
        if (typeof aviso_ === 'function') aviso_('Enlace copiado');
        else Swal.fire({ icon: 'success', title: 'Enlace copiado', timer: 1000, showConfirmButton: false });
      } catch (e) {
        Swal.fire({ icon: 'info', title: 'Copia el enlace', input: 'text', inputValue: d.url });
      }
    } else if (r.dismiss === Swal.DismissReason.cancel && admin) {
      ofePdf_(id, true);
    }
  } catch (e) {
    Swal.fire({ icon: 'error', title: 'No se pudo generar el PDF', text: String(e.message || e) });
  }
}
...
function ofeCampoHtml_(c) {
  ...
  if (c.tipo === 'imagen') {
    ...
    /* FASE 4.2 · punto 6 — los campos con tope llevan contador en
       vivo. El tope viaja en la definición del campo (OFE_CAMPOS), no
       está escrito aquí. */
    const cont = c.max ? `<div class="ofe-cont" id="ofe-cont-${c.id}"></div>` : '';
    control = `<textarea id="ofe-f-${c.id}" rows="3" class="ofe-input"${c.max ? ` data-max="${c.max}"` : ''}>${esc_(v)}</textarea>${cont}`;
...
/* FASE 4.2 · punto 6 — "185 / 270 caracteres": gris mientras haya
   margen, rojo al pasarse. El botón de guardar se bloquea igual que
   con los demás avisos de validación del formulario. */
function ofeContador_(id, max) {
  const ta = document.querySelector('#ofe-f-' + id);
  const cont = document.querySelector('#ofe-cont-' + id);
  if (!ta || !cont) return;
  const n = String(ta.value || '').trim().length;
  const pasado = n > max;
  cont.textContent = n + ' / ' + max + ' caracteres';
  cont.classList.toggle('ofe-cont--mal', pasado);
  ta.classList.toggle('ofe-input--mal', pasado);
  ofeRevisarTopes_();
}

/* Bloquea Guardar mientras algún campo con tope esté pasado. */
function ofeRevisarTopes_() {
  const btn = document.querySelector('#ofe-save');
  if (!btn) return;
  let mal = false;
  document.querySelectorAll('#ofe-modal-body textarea[data-max]').forEach(ta => {
    const max = parseInt(ta.getAttribute('data-max'), 10) || 0;
    if (max && String(ta.value || '').trim().length > max) mal = true;
  });
  btn.disabled = mal;
  btn.classList.toggle('btn--off', mal);
}
...
function cerrarModalOferta_() {
  ...
  const btn = document.querySelector('#ofe-save');
  if (btn) { btn.disabled = false; btn.classList.remove('btn--off'); }
  OFE.edit = null;
}
...
function ofeEngancharForm_() {
  ...
  /* FASE 4.2 · punto 6 — contadores en vivo de los campos con tope.
     Se pintan de entrada para que una descripción vieja que se pase
     lo diga al ABRIR la oferta, no al guardar. */
  cuerpo.querySelectorAll('textarea[data-max]').forEach(ta => {
    const id = ta.id.replace('ofe-f-', '');
    const max = parseInt(ta.getAttribute('data-max'), 10) || 0;
    ta.addEventListener('input', () => ofeContador_(id, max));
    ofeContador_(id, max);
  });
...
function ofeValidarFront_(datos) {
  ...
  campos.forEach(c => {
    ...
    /* FASE 4.2 · punto 6 — el mismo tope que valida el backend. */
    if (c.max && v.length > c.max) {
      errores.push({ id: c.id, label: c.label,
        motivo: 'Máximo ' + c.max + ' caracteres (van ' + v.length + ')' });
    }
  });
...
function pintarOfertasConfig_() {
  ...
  cont.innerHTML = `
    <div class="ofe-cfg-card">
      ...
      <div class="ofe-campo">
        <label for="ofecfg-plantillaPdf">ID de la plantilla</label>
        <div class="ofe-linea">
          <input id="ofecfg-plantillaPdf" class="ofe-input" type="text"
                 value="${esc_(c.plantillaPdf || '')}"${OFECFG.puedeEditar ? '' : ' disabled'} />
          <button type="button" class="file-btn" id="ofecfg-plantillaPdf-ir"${c.plantillaPdf ? '' : ' disabled'}>↗ Ir a plantilla</button>
        </div>
        <div class="ofe-hint">Presentación de Google con los marcadores del PDF de la oferta.
        El botón la abre en una pestaña nueva. Se puede pegar el ID suelto o la URL completa.</div>
      </div>
      ${ofeCfgCampo_('carpetaPdf', 'Carpeta de Drive para los PDF', c.carpetaPdf, 'Si se deja vacía se usa la carpeta OFERTAS_PDF del Drive de SEP.')}
      ${ofeCfgCampo_('notaPdf', 'Nota legal del final del PDF', c.notaPdf, 'Sale en la última página, bajo el título "IMPORTANT NOTE".', 'textarea')}
    </div>
    ...
    <div class="ofe-cfg-card">
      ...
      <div class="ofe-campo">
        <label for="ofecfg-diasConfirma">Plazo para confirmar una oferta propuesta por SEP (días)</label>
        <input id="ofecfg-diasConfirma" class="ofe-input" type="number" min="1" max="60"
               value="${esc_(c.diasConfirma || 7)}"${OFECFG.puedeEditar ? '' : ' disabled'} />
        <div class="ofe-hint">Cuando SEP aplica por el participante, el cupo queda <b>reservado</b> y él
        tiene este plazo para confirmar o rechazar. Si no responde, la propuesta se cancela sola y el
        cupo vuelve a estar disponible. Hoy: 7 días.</div>
      </div>
      <div class="ofe-campo">
  ...
  /* AJUSTE 03/09/2026 — botón "Ir a plantilla", igual que el de
     Configuración → Programas. Se habilita y se deshabilita MIENTRAS
     se escribe, sin tener que guardar primero: es la misma mecánica
     de app.js, así que si mañana cambia la forma de la URL de las
     presentaciones se cambia en un solo sitio (ofePlantillaUrl_). */
  const inp = document.querySelector('#ofecfg-plantillaPdf');
  const ir  = document.querySelector('#ofecfg-plantillaPdf-ir');
  if (inp && ir) {
    const refresco_ = () => { ir.disabled = !ofePlantillaUrl_(inp.value); };
    inp.addEventListener('input', refresco_);
    refresco_();
    ir.addEventListener('click', () => {
      const url = ofePlantillaUrl_(inp.value);
      if (!url) {
        Swal.fire({ icon: 'info', title: 'Sin plantilla', text: 'Todavía no hay ID de plantilla del PDF de la oferta.' });
        return;
      }
...
/* URL de la PRESENTACIÓN de Google con la plantilla del PDF. Acepta el
   ID suelto o la URL completa pegada, igual que en Programas. */
function ofePlantillaUrl_(valor) {
...
const OFE_CFG_TEXTOS = [
  ...
  /* FASE 4.1 (04/09/2026) — los cuatro textos nuevos. */
  ['txtPropuesta', 'Oferta propuesta por SEP', 'Lo que lee cuando SEP le asigna una oferta. Donde escribas [DIAS] se pone el plazo.'],
  ['txtRechazo', 'Antes de rechazar la propuesta', 'Se le muestra al pulsar "Rechazar": el cupo se libera y no vuelve a escoger por su cuenta.'],
  ['txtPuertaCurso', 'Puerta · curso de inglés sin confirmar', 'Aceptado con condición y sin curso confirmado: ve las ofertas pero no las escoge.'],
  ['txtPuertaEspera', 'Puerta · esperando propuesta de SEP', 'Rechazó la propuesta o dejó vencer el plazo: espera a que SEP le proponga otra.']
];

async function guardarOfertasConfig_() {
  if (!OFECFG.puedeEditar) return;
  const claves = ['plantillaPdf', 'carpetaPdf', 'notaPdf', 'carpetaFotos', 'programa',
    'temporada', 'politicaCupo', 'diasConfirma', 'listaEstadosUsa', 'listaPosiciones']
    .concat(OFE_CFG_TEXTOS.map(x => x[0]));
  const config = {};
  claves.forEach(k => {
...
/* FASE 4.1 — cuántos de los cupos ocupados están RESERVADOS (una
   propuesta esperando respuesta) y cuántos ya consumidos. */
function ofeCuposDetalleHtml_(c) {
  if (!c || !c.reservados) return '';
  return `<div class="ofe-part__aviso ofe-part__aviso--info">⏳ ${c.reservados} cupo(s) reservado(s)
    esperando la confirmación del participante · ${c.consumidos} consumido(s) de ${c.total}.
    Un cupo reservado no le aparece libre a nadie más.</div>`;
}

function ofePartCabecera_() {
  const d = OFE.part.datos;
  const c = d.cupos;
  /* Los nombres de las claves cambiaron con el lote Mis Bots
     (04/09/2026): ya no son BB2_*, es el Bot Procesos de Mis Bots. */
  const avisoBot = d.botProcesos ? '' :
    `<div class="ofe-part__aviso">⚠️ El <b>Bot Procesos</b> todavía no está conectado
       (Mis Bots → Bot Procesos, o sus claves en Configuración → Avanzado). Los avisos de
       esta pantalla saldrán <b>solo por correo</b>.</div>`;
  return `<div class="ofe-part__head">
    ...
    </div>${avisoBot}${ofeCuposDetalleHtml_(c)}`;
}
...
function ofePartFilaHtml_(p) {
  ...
  if (p.puede.liberarCupo) acciones.push(`<button class="btn btn-ghost" data-pact="liberar" data-id="${esc_(p.id)}">🎟️ Liberar cupo</button>`);
  ...
  /* FASE 4.1 — la propuesta que espera respuesta del participante:
     ...
     de entrevista ni de resultado (lo decide el objeto `puede` del
     backend, no esta pantalla). */
  const conf = p.porConfirmar
    ? `<div class="ofe-part__conf">⏳ Esperando la confirmación del participante · vence el
        <b>${esc_(String(p.vence || '').slice(0, 10))}</b>${
        (p.diasRestantes === null || p.diasRestantes === undefined) ? ''
          : ` (${Math.max(0, p.diasRestantes)} día(s))`}
        <br><span class="muted">El cupo está reservado: no aparece libre para nadie más.</span></div>`
    : '';
  ...
  return `<article class="ofe-part__card" data-pid="${esc_(p.id)}">
    ...
    <div class="ofe-part__meta">
      ...
      <span>🎟️ ${p.cupoReservado ? 'cupo reservado'
        : (p.cupoConsumido && !p.cupoDevuelto ? 'cupo consumido'
        : (p.cupoDevuelto ? 'cupo devuelto' : 'sin cupo'))}</span>
    </div>
    ${conf}${ent}${res}${exc}${hab}${rech}
...
async function ofePartAccion_(accion, id) {
  ...
  if (accion === 'liberar')    return ofePartLiberarCupo_(p);
}
...
/* AJUSTE 09/09/2026 — LIBERAR EL CUPO (en silencio)
   Solo aparece cuando el backend dice que hay un cupo retenido: la
   entrevista terminó en NO APROBADA y esa oferta tenía la política
   "el cupo se pierde". El participante sigue como No aprobado y no
   se le avisa nada; el cupo vuelve a contar como disponible.      */
async function ofePartLiberarCupo_(p) {
  const r = await Swal.fire({
    icon: 'question', title: 'Liberar el cupo', width: 600,
    html: `<div style="text-align:left;font-size:14px">
        <p><b>${esc_(p.nombre)}</b> · ${esc_(p.empleador || '')} — ${esc_(p.posicion || '')}</p>
        <p>Su cupo quedó ocupado porque esta oferta tenía la política
           <b>el cupo se pierde</b>. Al liberarlo, la oferta vuelve a
           sumar ese cupo entre los disponibles.</p>
        <p style="font-size:12.5px;color:#64748b">
           El participante sigue como <b>No aprobado</b> y
           <b>no se le envía ningún aviso</b>. Queda escrito en el
           historial de la oferta y en el de esta aplicación.</p>
      </div>`,
    showCancelButton: true, confirmButtonText: 'Liberar el cupo', cancelButtonText: 'Cancelar'
  });
  if (!r.isConfirmed) return;
  try {
    const d = await apiPost('liberarCupo', { usuarioId: currentUser.id, id: p.id });
    await recargarParticipantes_(true);
    await recargarOfertas_(true);
    Swal.fire({ icon: 'success', title: 'Cupo liberado',
      text: (d && d.cupo) ? String(d.cupo) : '', timer: 1600, showConfirmButton: false });
  } catch (e) {
