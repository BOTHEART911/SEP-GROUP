...
async function recargarContador_(silencioso) {
  try {
    CONTA.registros = await apiGet('listContador', { usuarioId: currentUser.id });
    renderContaFiltros_(); renderContaCards_(); renderContaResumen_();
  } catch (e) {
...
function abrirModalContador_(r) {
  ...
  document.querySelector('#conta-modal-body').innerHTML = `
    ...
    <details class="conta-bloque">
      ...
      <div class="form-grid">
        <div class="fld fld-full"><label class="conta-chk"><input type="checkbox" id="c-contratoOk"${r.contratoOk ? ' checked' : ''}/><span>Contrato OK (revisado y correcto)</span></label></div>
...
async function subirComprobante_(e) {
  ...
  try {
    ...
    const r = await apiPost('subirComprobante', {
      ...
      filename: file.name, mime: file.type, base64: base64
    });
    document.querySelector('#c-comprobanteUrl').value = r.url;
    document.querySelector('#c-comprobante-est').textContent = 'Cargado — falta guardar';
