...
function abrirModalContador_(r) {
    ...
    <details class="conta-bloque">
      ...
      <div class="form-grid">
        ...
        <div class="fld fld-full conta-archivos">
          ${r.contratoUrl  ? `<button type="button" class="btn btn-ghost" data-ver="${esc_(r.contratoUrl)}" data-titulo="Contrato firmado">📄 Ver contrato</button>
                              <button type="button" class="btn btn-ghost" data-bajar="${esc_(r.contratoUrl)}">⬇ Descargar</button>` : '<span class="conta-hint">Contrato aún no creado por el estudiante</span>'}
          ${r.documentoUrl ? `<button type="button" class="btn btn-ghost" data-ver="${esc_(r.documentoUrl)}" data-titulo="Documento del estudiante">🪪 Ver documento</button>
                              <button type="button" class="btn btn-ghost" data-bajar="${esc_(r.documentoUrl)}">⬇ Descargar</button>` : ''}
          ${r.cedulaUrl    ? `<button type="button" class="btn btn-ghost" data-ver="${esc_(r.cedulaUrl)}" data-titulo="Cédula del deudor">🧑‍🤝‍🧑 Ver cédula</button>
                              <button type="button" class="btn btn-ghost" data-bajar="${esc_(r.cedulaUrl)}">⬇ Descargar</button>` : ''}
        </div>
  ...
  document.querySelectorAll('#conta-modal-body [data-ver]').forEach(b =>
    b.addEventListener('click', () => abrirVisorConta_(b.dataset.ver, b.dataset.titulo)));
  document.querySelectorAll('#conta-modal-body [data-bajar]').forEach(b =>
    b.addEventListener('click', () => contaDescargar_(b.dataset.bajar)));
...
function contaZonaPintarLista_(id) {
  ...
  cont.innerHTML = z.urls.map((u, i) => `
    <div class="cfz-item">
      ...
      <span class="cfz-item__b">
        ...
        <button type="button" class="act-btn" data-cfz="descargar" data-i="${i}">⬇ Descargar</button>
  ...
  cont.querySelectorAll('[data-cfz]').forEach(b => b.addEventListener('click', () => {
    ...
    else if (b.dataset.cfz === 'descargar') contaDescargar_(url);
