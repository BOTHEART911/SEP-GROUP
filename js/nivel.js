/* ============================================================
 * SEP GROUP — VISTA NIVEL DE INGLÉS (Fase 3 SEP · Entrega 2 · 16/08/2026)
 ...
 *
 * Lo que está a propósito DESHABILITADO (llega en las Entregas 4 y 5):
 *   el check "Aprobado", "Ver formulario" y "Ver HV". Se pintan para
 *   que se vea dónde van, con su explicación al tocarlos.
 *
...
function niveValFiltro_(k) { return k === 'estado' ? NIVE.filtroEstado : NIVE.filtroNivel; }
function niveSetFiltro_(k, v) {
  if (k === 'estado') { NIVE.filtroEstado = v; NIVE.filtroNivel = '__ALL__'; }
  else NIVE.filtroNivel = v;
}
...
function niveTotalFiltro_(k) { return k === 'estado' ? NIVE.registros.length : niveBaseEstado_().length; }
function niveConteoPill_(k)  { return k === 'estado' ? niveBaseEstado_().length : niveBaseNivel_().length; }
...
function renderNiveCards_() {
  ...
  let list = niveBaseNivel_().slice().sort((a, b) => b.n - a.n);
  ...
  list.forEach((r, i) => {
    ...
    card.querySelectorAll('[data-pronto]').forEach(b =>
      b.addEventListener('click', () => niveProntoAviso_(b.dataset.pronto)));
  });
}

/* Los botones que llegan en las Entregas 4 y 5 explican por qué están
   apagados en vez de no hacer nada. */
function niveProntoAviso_(que) {
  const m = {
    formulario: { t: 'Ver formulario', h: 'El formulario del estudiante se construye en las <b>Entregas 3 y 4</b>. Cuando esté, este botón lo abre por secciones y permite desbloquear bloques.' },
    hv: { t: 'Ver hoja de vida', h: 'La hoja de vida se genera en la <b>Entrega 5</b>, al marcar <b>Aprobado</b>. Este botón la abrirá sin salir de la app, con descargar y copiar enlace.' },
    aprobado: { t: 'Check Aprobado', h: 'Marcar <b>Aprobado</b> crea la hoja de vida y manda el mensaje de ofertas de empleo. Se habilita en la <b>Entrega 5</b>, cuando el formulario ya se pueda validar.' }
  }[que] || { t: 'Próxima entrega', h: 'Esta parte llega en una entrega siguiente.' };
  Swal.fire({ icon: 'info', title: m.t, html: m.h });
}

function niveCardHtml_(r, i) {
  const p = nivePuntajeTexto_(r.puntaje);
  return `<div class="com-card nive-card" id="nive-card-i${i}" data-n="${r.n}">
    ...
    <div class="nive-card__top">
      ...
      <div class="com-card__head">
        ...
        <div class="com-card__meta">
          ...
          ${r.nacimiento ? `<span>🎂 ${esc_(r.nacimiento)}</span>` : ''}
        </div>
      </div>
    ...
    <div class="com-card__actions">
      ...
      <button class="act-btn is-pronto" data-pronto="formulario">📝 Ver formulario</button>
      <button class="act-btn is-pronto" data-pronto="hv">🪪 Ver HV</button>
    </div>
  </div>`;
}
...
function abrirFotoNivel_(r) {
  ...
  document.querySelector('#nive-foto-nota').innerHTML = String(r.fotoUrl || '').trim()
    ? ''
    : 'Todavía no hay foto: el estudiante la sube en el <b>Bloque 14</b> del formulario (Entrega 4).';
  niveZoomReset_();
...
function abrirModalNivel_(r) {
  ...
  document.querySelector('#nive-modal-body').innerHTML = `
    ...
    <details class="conta-bloque">
      <summary>✅ Aprobación del perfil <small>(Entrega 5)</small></summary>
      <div class="form-grid">
        <div class="fld fld-full">
          <label class="conta-chk bloqueado">
            <input type="checkbox" id="nive-aprobado" disabled ${r.aprobado ? 'checked' : ''} />
            <span>Aprobado (crea la hoja de vida y avisa de las ofertas)</span></label>
          <small class="conta-hint">Se habilita cuando el formulario del estudiante se pueda validar. Toca el botón para ver el detalle.</small>
          <div class="nive-pronto-btns">
            <button type="button" class="btn btn-ghost is-pronto" data-pronto="aprobado">ℹ️ Por qué está bloqueado</button>
            <button type="button" class="btn btn-ghost is-pronto" data-pronto="formulario">📝 Ver formulario</button>
            <button type="button" class="btn btn-ghost is-pronto" data-pronto="hv">🪪 Ver HV</button>
          </div>
        </div>
      </div>
    </details>`;
  ...
  document.querySelectorAll('#nive-modal-body [data-pronto]').forEach(b =>
    b.addEventListener('click', () => niveProntoAviso_(b.dataset.pronto)));
...
async function guardarPuntajeNivel_() {
  ...
  try {
    const out = await apiPost('guardarPuntaje', {
      usuarioId: currentUser.id, n: r.n, documento: r.documento, puntaje: crudo
    });
    cerrarModalNivel_();
...
function niveOverlayAbierto_() {
  ...
  if (abierto('#modal-nivel') || abierto('#nive-foto-modal') || abierto('#nive-fsheet')) return true;
