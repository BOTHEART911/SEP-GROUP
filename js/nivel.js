...
const NIVE = {
  ...
  /* FASE 4 · ENTREGA 5 — punto 2.3 (los retirados salen de la vista
     principal), 3.2 (filtro por Asesor de Procesos, solo Superadmin)
     y 3.4 (los cuatro indicadores del tablero, que funcionan como
     filtro). Los tres nacen apagados en cada entrada. */
  verRetirados: false,
  filtroAsesorProc: '__ALL__',
  filtroIndicador: ''
};
...
/* FASE 4 · ENTREGA 5 · 3.2 — la quinta pastilla, solo para quien ve a
   todos. Un usuario de PROCESOS no la necesita: el servidor ya le
   manda únicamente los suyos. */
const NIVE_FILTRO_ASESOR = { key: 'asesorProc', allLabel: 'Todos los asesores',
  titulo: 'Filtrar por Asesor de Procesos', ic: '🧭', color: '#7c3aed' };
const NIVE_SIN_ASESOR = '— Sin asesor —';

function niveVeTodos_() { return !!(NIVE.catalogo?.permisos?.verTodos); }
function nivePuedeRetirar_() { return !!(NIVE.catalogo?.permisos?.retirar); }
/* FASE 4.1 · punto 5 — reactivar quedó SOLO en Superadmin (y DEV). A
   quien no puede no se le deja un botón muerto: se le dice a quién
   pedírselo. */
...
function niveAsesorDe_(r) { return String(r.asesorProcesos || '').trim() || NIVE_SIN_ASESOR; }
...
function niveVisibles_() {
  ...
  if (NIVE.filtroIndicador && NIVE.filtroIndicador !== 'RETIRADOS') {
    base = base.filter(r => r.indicador === NIVE.filtroIndicador);
  }
  return base;
}
...
function niveValFiltro_(k) {
  if (k === 'asesorProc') return NIVE.filtroAsesorProc;   // ENTREGA 5 · 3.2
...
function niveSetFiltro_(k, v) {
  ...
  if (k === 'asesorProc') {
    /* Va antes que todas: al cambiar de asesor se reinician las
       cuatro pastillas de la derecha. */
    NIVE.filtroAsesorProc = v; NIVE.filtroEstado = '__ALL__'; NIVE.filtroNivel = '__ALL__';
...
function niveBaseEstado_() {
  const base = niveVisibles_();   // ENTREGA 5 — retirados · asesor · indicador
  if (NIVE.filtroEstado === '__ALL__') return base;
  return base.filter(r => r.estado === NIVE.filtroEstado);
}
...
function niveOpciones_(key) {
  ...
  if (key === 'estado') {
    niveVisibles_().forEach(r => { c[r.estado] = (c[r.estado] || 0) + 1; });
...
function niveConteoPill_(k) {
  if (k === 'asesorProc') return niveVisibles_().length;
...
function renderNiveFiltros_() {
  ...
  cont.innerHTML = niveFiltrosVisibles_().map(nivePillHtml_).join('');
  niveFiltrosVisibles_().forEach(f =>
...
function abrirNiveSheet_(key) {
  const f = niveFiltrosVisibles_().find(x => x.key === key); if (!f) return;
...
function renderNiveResumen_() {
  ...
  const conP = niveVisibles_().filter(r => r.puntaje !== '' && r.puntaje !== null && r.puntaje !== undefined);
  if (conP.length) {
  ...
  renderNiveIndicadores_();
}

/* ============================================================
   FASE 4 · ENTREGA 5 · punto 3.4 — TABLERO DE PROCESOS
   ============================================================
   Cuatro indicadores rápidos que además son filtros: Sin oferta ·
   Entrevista pendiente · Entrevista agendada · Retirados. No hay uno
   de "Oferta seleccionada" a propósito (lo dice el plan): una oferta
   seleccionada o aplicada entra en "Entrevista pendiente".
   "Retirados" es el que cambia de lista; los otros tres filtran
   dentro de los activos. */
function renderNiveIndicadores_() {
  const cont = document.querySelector('#nive-proc'); if (!cont) return;
  const defs = NIVE.catalogo?.indicadores || [];
  if (!defs.length) { cont.innerHTML = ''; return; }

  const activos = NIVE.registros.filter(r => !r.retirado);
  const cuenta = {};
  activos.forEach(r => { if (r.indicador) cuenta[r.indicador] = (cuenta[r.indicador] || 0) + 1; });
  const retirados = NIVE.registros.filter(r => r.retirado).length;

  cont.innerHTML = defs.map(d => {
    const esRet = d.clave === 'RETIRADOS';
    const n = esRet ? retirados : (cuenta[d.clave] || 0);
    const on = esRet ? NIVE.verRetirados : (NIVE.filtroIndicador === d.clave);
    return `<button class="conta-kpi nive-ind${on ? ' is-on' : ''}" data-ind="${d.clave}" style="--k:${d.color}">
      <span class="conta-kpi__n">${n}</span>
      <span class="conta-kpi__t">${d.ic} ${esc_(d.label)}</span></button>`;
  }).join('');

  cont.querySelectorAll('[data-ind]').forEach(b => b.addEventListener('click', () => {
    const k = b.dataset.ind;
    if (k === 'RETIRADOS') {
      NIVE.verRetirados = !NIVE.verRetirados;
      NIVE.filtroIndicador = '';
    } else {
      NIVE.filtroIndicador = (NIVE.filtroIndicador === k) ? '' : k;
      NIVE.verRetirados = false;
    }
    /* Cambiar de lista reinicia las pastillas: si no, se quedaría un
       filtro puesto sobre un grupo que ya no existe. */
    NIVE.filtroAsesorProc = '__ALL__'; NIVE.filtroEstado = '__ALL__';
    NIVE.filtroNivel = '__ALL__'; NIVE.filtroInvitacion = '__ALL__'; NIVE.filtroOferta = '__ALL__';
    renderNiveResumen_(); renderNiveFiltros_(); renderNiveCards_();
  }));
}
...
function renderNiveCards_() {
  ...
  list.forEach((r, i) => {
    ...
    /* FASE 4 · ENTREGA 6 — los 10 documentos del participante (4.10) */
    card.querySelector('[data-act="docs"]')?.addEventListener('click', () => niveVerDocs_(r));
    /* AJUSTE 3 y 4 (19/08/2026) */
    card.querySelector('[data-act="invitacion"]')?.addEventListener('click', () => niveInvitacion_(r));
    /* FASE 4.1 · punto 6 */
    card.querySelector('[data-act="curso"]')?.addEventListener('click', () => niveCurso_(r));
...
/* ============================================================
   FASE 4 · ENTREGA 5 — RETIRAR / REACTIVAR / CAMBIAR ASESOR
   ============================================================
   Las mismas acciones de la vista Contador, desde la ficha de
   Procesos: quien trabaja al participante todos los días es quien se
   entera de que se retiró. El N° con el que viajan es el de la hoja
   CONTADOR (nContador), no el de NIVEL_INGLES: son consecutivos
   distintos y confundirlos retiraría al que no era. */
function niveNContador_(r) {
  return Number(r && r.nContador) || 0;
}

async function niveRetirar_(r, retirar) {
  const n = niveNContador_(r);
  if (!n) {
    Swal.fire({ icon: 'info', title: 'Sin ficha en Contador',
      text: 'Este estudiante no tiene fila en la vista Contador, así que no se puede retirar desde aquí.' });
    return;
  }
  const { value: motivo } = await Swal.fire({
    icon: retirar ? 'warning' : 'question',
    title: retirar ? 'Retirar participante' : 'Reactivar participante',
    html: retirar
      ? `<b>${esc_(r.nombres)} ${esc_(r.apellidos)}</b> sale de la vista principal y deja de recibir mensajes
         automáticos.<br><small>No se borra nada. Si tiene una oferta activa, se cancela y el cupo se libera.</small>`
      : `<b>${esc_(r.nombres)} ${esc_(r.apellidos)}</b> vuelve a las listas normales.
         <br><small>La oferta que tenía al retirarse no se le devuelve: vuelve a escoger desde cero.</small>`,
    input: 'textarea',
    inputLabel: retirar ? 'Motivo del retiro (obligatorio)' : 'Motivo de la reactivación (obligatorio)',
    showCancelButton: true, confirmButtonText: retirar ? 'Retirar' : 'Reactivar', cancelButtonText: 'Cancelar',
    inputValidator: v => (String(v || '').trim().length < 5) && 'Escribe el motivo (mínimo 5 caracteres).'
  });
  if (!motivo) return;
  try {
    const out = await apiPost(retirar ? 'retirarParticipante' : 'reactivarParticipante',
      { usuarioId: currentUser.id, n: n, motivo: motivo });
    await recargarNivel_(true);
    Swal.fire({ icon: 'success', title: retirar ? 'Participante retirado' : 'Participante reactivado',
      html: (retirar && out.oferta && out.oferta.habia)
        ? `Su oferta <b>${esc_(out.oferta.empleador || out.oferta.oferta)}</b> quedó cancelada y ${esc_(out.oferta.cupo)}.`
        : '' });
  } catch (e) {
...
async function niveCambiarAsesor_(r) {
  ...
  if (!n) {
    Swal.fire({ icon: 'info', title: 'Sin ficha en Contador',
      text: 'El Asesor de Procesos se guarda en la ficha del Contador y este estudiante todavía no la tiene.' });
    return;
  }
  const lista = (NIVE.catalogo?.asesoresProcesos || []).filter(a => a.id !== r.asesorProcesosId);
  if (!lista.length) {
  ...
  const opciones = {};
  lista.forEach(a => { opciones[a.id] = a.nombre; });
  const { value: elegido } = await Swal.fire({
    title: r.asesorProcesos ? 'Cambiar Asesor de Procesos' : 'Asignar Asesor de Procesos',
    html: `Asesor actual: <b>${esc_(r.asesorProcesos || '— sin asesor —')}</b>`,
    input: 'select', inputOptions: opciones, inputPlaceholder: 'Elige el asesor',
    showCancelButton: true, confirmButtonText: 'Continuar',
    inputValidator: v => !v && 'Elige el asesor.'
  });
  if (!elegido) return;
  const { value: motivo } = await Swal.fire({
    title: 'Motivo', input: 'textarea',
  ...
  try {
    const out = await apiPost('asignarAsesorProcesos',
      { usuarioId: currentUser.id, n: n, asesorId: elegido, motivo: motivo || '' });
    await recargarNivel_(true);
    Swal.fire({ icon: 'success', title: 'Asesor guardado',
...
/* FASE 4 · ENTREGA 6 — puente hacia NDOCS (js/nivel-docs.js), los 10
   documentos del participante con sus 5 estados (punto 4.10). */
function niveHayDocs_() { return (typeof NDOCS !== 'undefined') && !!NDOCS; }
function niveVerDocs_(r) {
  if (!niveHayDocs_()) {
    return Swal.fire({
      icon: 'error', title: 'Falta una parte de la app',
      html: 'No se cargó <b>js/nivel-docs.js</b>. Recarga la pantalla; si sigue igual, avísale al desarrollador.'
    });
  }
  NDOCS.abrir(r);
}
...
function niveCardHtml_(r, i) {
  ...
  return `<div class="com-card nive-card${r.invitacion ? ' nive-card--inv' : ''}${r.retirado ? ' nive-card--retirado' : ''}" id="nive-card-i${i}" data-n="${r.n}">
    ...
    <div class="com-card__actions">
      ...
      <button class="act-btn" data-act="docs">📁 Documentos</button>
      ...
      ${r.cursoAplica ? `<button class="act-btn${r.curso ? ' act-btn--ok' : ''}" data-act="curso"
        title="${r.curso
          ? 'Quitar la confirmación del curso de inglés: volverá a no poder seleccionar ofertas.'
          : 'Confirmar que está haciendo el curso de inglés obligatorio. Sin esto puede VER las ofertas pero no seleccionarlas.'}">🗣️ ${r.curso ? 'Curso confirmado' : 'Confirmar curso'}</button>` : ''}
      ${nivePuedeReasignar_() ? '<button class="act-btn" data-act="asesor">🧭 Asesor</button>' : ''}
      ${nivePuedeRetirar_()
          ? (r.retirado
              ? (nivePuedeReactivar_()
...
/* FASE 4 · ENTREGA 5 — lo que la tarjeta dice de Procesos: el asesor
   (3.2), el indicador del tablero (3.4) y, en rojo, el retiro (2.3).
   Los tres salen de CONTADOR y de OFERTAS_SELECCION: aquí no se
   guarda ninguno. */
function niveProcHtml_(r) {
  const partes = [];
  partes.push(r.asesorProcesos
    ? `<span class="nive-res__apr">🧭 ${esc_(r.asesorProcesos)}</span>`
    : `<span class="nive-res__apr falta" title="Se asigna al confirmar el comprobante de inscripción.">🧭 sin asesor</span>`);
  if (r.retirado) {
    partes.push(`<span class="nive-res__retiro">🛑 RETIRADO${r.retiroFecha ? ' · ' + esc_(r.retiroFecha) : ''}</span>`);
  } else {
    const def = (NIVE.catalogo?.indicadores || []).find(d => d.clave === r.indicador);
    if (def) partes.push(`<span class="nive-res__ind" style="--k:${def.color}">${def.ic} ${esc_(def.label)}</span>`);
    if (r.seleccion && r.seleccion.empleador) {
      partes.push(`<span class="nive-res__ofe">💼 ${esc_(r.seleccion.empleador)}</span>`);
    }
  }
  return partes.join('');
}

/* FASE 4.1 · PUNTO 6 — el curso de inglés, en la franja de resultado.
   Solo se pinta a quien le aplica (ACEPTADO_CONDICION): a los demás
   no les dice nada y ensuciaría la tarjeta. En ámbar cuando falta,
   porque mientras falte esa persona NO puede seleccionar oferta. */
function niveCursoHtml_(r) {
  if (!r || !r.cursoAplica) return '';
  if (!r.curso) {
    return `<span class="nive-flag curso-no" title="Aceptado con condición: puede ver las ofertas pero no seleccionarlas hasta que SEP confirme el curso de inglés.">🗣️ Curso de inglés sin confirmar</span>`;
  }
  const quien = String(r.cursoPor || '').trim();
  const cuando = String(r.cursoFecha || '').trim();
  const detalle = (quien || 'Confirmado') + (cuando ? ' · ' + cuando : '');
  return `<span class="nive-flag curso-si" title="Curso de inglés confirmado por ${esc_(detalle)}">🗣️ Curso confirmado · ${esc_(detalle)}</span>`;
}
...
/* ============================================================
   FASE 4.1 · PUNTO 6 (04/09/2026) — CURSO DE INGLÉS CONFIRMADO
   ============================================================
   Mismo interruptor que la invitación, pero este SÍ cambia lo que el
   participante puede hacer: sin él, quien quedó "aceptado con
   condición" ve las ofertas y no puede seleccionarlas. Por eso se
   confirma antes de marcarlo y se dice qué implica. */
async function niveCurso_(r) {
  const marcar = !r.curso;
  const ok = await Swal.fire({
    icon: 'question',
    title: marcar ? 'Confirmar el curso de inglés' : 'Quitar la confirmación',
    html: `<b>${esc_(r.nombres)} ${esc_(r.apellidos)}</b><br><small>${marcar
      ? 'Al confirmar que está haciendo el curso de inglés obligatorio, podrá <b>seleccionar</b> ofertas de empleo (si cumple lo demás).'
      : 'Al quitarlo dejará de poder seleccionar ofertas: solo podrá verlas.'}</small>`,
    showCancelButton: true,
    confirmButtonText: marcar ? 'Confirmar curso' : 'Quitar confirmación',
    cancelButtonText: 'Cancelar'
  });
  if (!ok.isConfirmed) return;
  try {
    const out = await apiPost('nivelCursoIngles', {
      usuarioId: currentUser.id, n: r.n, documento: r.documento, marcar: marcar
    });
    if (out && out.registro) {
      const i = NIVE.registros.findIndex(x => x.n === r.n);
    ...
    renderNiveFiltros_(); renderNiveResumen_(); renderNiveCards_();
    Swal.fire({ icon: 'success', title: marcar ? 'Curso confirmado' : 'Confirmación quitada',
      timer: 1200, showConfirmButton: false });
  } catch (e) {
    Swal.fire({ icon: 'error', title: 'No se pudo cambiar el curso de inglés', text: String(e.message || e) });
  }
}
...
/* 05/09/2026 — el visor deja de ser solo del estudiante: recibe título,
   foto y nota, así lo reutiliza el chip del usuario en la cabecera y las
   tarjetas de Usuarios sin inventar un segundo visor. La puerta de los
   estudiantes (abrirFotoNivel_) sigue igual. */
function abrirFotoZoom_(opts) {
  const o = opts || {};
  const ov = document.querySelector('#nive-foto-modal'); if (!ov) return;
  document.querySelector('#nive-foto-title').textContent = String(o.titulo || 'Foto');
  const img = document.querySelector('#nive-foto-img');
  img.onerror = function () { this.onerror = null; this.src = USR_FOTO_FALLBACK; };
  img.alt = String(o.alt || o.titulo || 'Foto');
  img.src = String(o.url || USR_FOTO_FALLBACK);
  document.querySelector('#nive-foto-nota').innerHTML = String(o.nota || '');
  niveZoomReset_();
  ov.classList.remove('hidden');
}
window.sepFotoZoom = abrirFotoZoom_;

function abrirFotoNivel_(r) {
  abrirFotoZoom_({
    titulo: (r.nombres + ' ' + r.apellidos).trim() + ' · N° ' + r.n,
    alt: 'Foto de ' + (r.nombres || ''),
    url: niveFoto_(r),
    nota: String(r.fotoUrl || '').trim()
      ? ''
...
/* FASE 3.1 · ajuste 5 — las 4 opciones del "Nivel de Inglés HV".
   ...
   no está publicado. */
/* FASE 3.4 (21/08/2026) — las opciones pasaron a Capitalizado y ahora se
   administran en Configuración → Listas, así que las de verdad son las
   que manda el backend en el catálogo. Esto es solo el respaldo. */
...
/* Comparación del nivel guardado contra las opciones de la lista.
   FASE 3.4 — el select marcaba la opción con `===`: una ficha guardada
   como "INTERMEDIATE" contra la opción "Intermediate" abría el modal SIN
   nada seleccionado y el contador tenía que volver a elegir a ciegas.
   Se compara sin mayúsculas, tildes ni guiones, igual que el backend. */
function niveNormHv_(v) {
  return String(v == null ? '' : v)
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .toUpperCase().replace(/[^A-Z0-9]/g, '');
}
function niveHvSeleccionado_(guardado, opcion) {
  const g = niveNormHv_(guardado);
  return !!g && g === niveNormHv_(opcion);
}
...
/* AJUSTE 25/08/2026 — rótulo del botón Guardar según el silencio. */
function niveRotuloGuardar_(silencio) {
  const b = document.querySelector('#nive-save');
  if (b) b.textContent = silencio ? 'Guardar sin enviar' : 'Guardar y enviar';
}
...
function abrirModalNivel_(r) {
      ...
      <div class="form-grid">
        ...
        <div class="fld">
          ...
          <select id="nive-nivelhv">
            ...
            ${niveOpcionesHv_().map(o => `<option value="${esc_(o)}" ${niveHvSeleccionado_(r.nivelHv, o) ? 'selected' : ''}>${esc_(o)}</option>`).join('')}
          </select>
        ...
        <div class="fld fld-full">
          <label class="conta-chk">
            <input type="checkbox" id="nive-silencio" />
            <span>🔕 Guardar en silencio</span></label>
          <small class="conta-hint">Guarda el <b>Puntaje SEA</b> y el <b>Nivel de Inglés HV</b>
            <b>sin enviar</b> WhatsApp ni correo, y no cuenta como envío. Sirve para montar
            resultados atrasados. No afecta el check <b>Aprobado</b>, que avisa por su cuenta.
            Nace apagado cada vez que se abre esta ficha.</small>
        </div>
        <div class="fld fld-full">
  ...
  /* El botón del pie es fijo en el HTML: su rótulo se pone aquí, y se
     devuelve a "Guardar y enviar" en CADA apertura, porque el check de
     silencio nace apagado siempre. */
  niveRotuloGuardar_(false);
  document.querySelector('#nive-silencio')?.addEventListener('change', e => {
    niveRotuloGuardar_(!!e.target.checked);
  });
      ...
      <div class="form-grid">
        <div class="fld fld-full">
          ...
          <div class="nive-aprob__btns">
            ...
            <button type="button" class="btn btn-ghost" data-nive="ofertas">💼 Ofertas</button>
          </div>
...
function niveCablearAprobacion_(r) {
  ...
  /* FASE 4 · ENTREGA 3 — el segundo camino del 1.26: asignar una
     oferta SIN salir de la ficha del participante. Va envuelto: si
     js/ofertas.js no cargó, el botón lo dice en vez de romper. */
  document.querySelector('#nive-modal-body [data-nive="ofertas"]')
    ?.addEventListener('click', () => {
      if (typeof abrirOfertasDeParticipante_ !== 'function') {
        Swal.fire({ icon: 'info', title: 'Ofertas',
          text: 'El módulo de ofertas de empleo no está disponible en esta pantalla.' });
        return;
      }
      abrirOfertasDeParticipante_(r.idRegistro || r.id, (r.nombres || '') + ' ' + (r.apellidos || ''));
    });
...
async function guardarPuntajeNivel_() {
  ...
  /* AJUSTE 25/08/2026 — "Guardar en silencio": mismo guardado, sin
     mensaje. La confirmación cambia entera, porque lo que se confirma
     es otra cosa (aquí no hay nada que no se pueda recoger). */
  const silencio = !!document.querySelector('#nive-silencio')?.checked;
  ...
  if (r.correo)   destinos.push('correo <b>' + esc_(r.correo) + '</b>');
  const res = await Swal.fire({
    title: silencio ? '¿Guardar sin enviar?'
                    : (r.envios ? '¿Volver a enviar el resultado?' : '¿Guardar y enviar el resultado?'),
    html: `<div class="nive-conf">
        <div class="nive-conf__q"><b>${esc_(r.nombres + ' ' + r.apellidos)}</b> · N° ${r.n}</div>
        ...
        <div class="nive-conf__d">${silencio
          ? '🔕 <b>No se envía ningún mensaje</b>: el estudiante no se entera de este resultado.'
          : (destinos.length
        ...
        ${(!silencio && r.envios) ? `<div class="nive-conf__re">Ya se le había enviado ${r.envios} ${r.envios === 1 ? 'vez' : 'veces'}: este sería el envío ${r.envios + 1}.</div>` : ''}
      </div>`,
    showCancelButton: true,
    confirmButtonText: silencio ? 'Sí, guardar en silencio'
                                : (r.envios ? 'Sí, reenviar' : 'Sí, guardar y enviar'),
    cancelButtonText: 'Cancelar',
    focusCancel: true
  });
  if (!res.isConfirmed) return;
  ...
  try {
    ...
    const a = out.aviso || {};
    if (out.silencio) {
      /* Quedó todo guardado, pero el estudiante no se enteró. */
      Swal.fire({
        icon: 'success', title: 'Guardado en silencio',
        html: `<b>${esc_(out.nombres + ' ' + out.apellidos)}</b> quedó en <b>${esc_(out.nivel)}</b> (${esc_(out.estadoLabel)}),
               con Nivel de Inglés HV <b>${esc_(out.nivelHv)}</b>.<br>
               <b>No se envió ningún mensaje</b> y el conteo de envíos quedó igual.`
      });
    } else if (a.enviado) {
