...
(function () {
  ...
  /* NIVEL DE INGLÉS (Fase 3 SEP · Entrega 2) — tras los filtros de
     resultado y nivel y el buscador, exactamente como
     renderNiveCards_. Las consultas de esta pantalla son PROPIAS: aquí
     no interesan asesores ni plata, sino el resultado de la prueba. */
  function nivel() {
    /* AJUSTE 3 (19/08/2026) — se lee la ÚLTIMA base de la cadena, que
       ya trae aplicada la pastilla de invitación. Si el archivo de la
       vista fuese viejo y no tuviera esa función, se cae a la
       anterior y todo sigue igual que antes. */
    /* FASE 3.3 · tanda B — la última de la cadena ahora es la del pago
       de la oferta. Se sigue cayendo hacia atrás si el archivo de la
       vista fuese viejo. */
    var lista = leer(function () { return niveBaseOferta_(); }, null);
    if (!lista) lista = leer(function () { return niveBaseInvitacion_(); }, null);
    if (!lista) lista = leer(function () { return niveBaseNivel_(); }, null);
    if (!lista) lista = leer(function () { return NIVE.registros; }, []) || [];
    var t = txt(leer(function () { return NIVE.filtroTexto; }, ''));
    if (!t) return lista.slice();
    var q = leer(function () { return niveNormBusq_(t); }, llano(t));
    return lista.filter(function (r) {
      var n = leer(function () { return niveNormBusq_(r.nombres + ' ' + r.apellidos); }, llano(r.nombres + ' ' + r.apellidos));
      return n.indexOf(q) >= 0 ||
        String(r.documento || '').indexOf(q) >= 0 ||
        String(r.whatsapp || '').indexOf(q) >= 0 ||
        String(r.n).indexOf(q) >= 0 ||
  ...
  function nivelFiltrosPuestos() {
    var f = [];
    var e = leer(function () { return NIVE.filtroEstado; }, '__ALL__');
    var n = leer(function () { return NIVE.filtroNivel; }, '__ALL__');
    var t = txt(leer(function () { return NIVE.filtroTexto; }, ''));
    if (e && e !== '__ALL__') {
      var def = leer(function () { return niveEstadoDef_(e); }, null);
      f.push('resultado ' + ((def && def.label) || e));
    }
    if (n && n !== '__ALL__') f.push('nivel ' + n);
    /* AJUSTE 3 (19/08/2026) */
    var iv = leer(function () { return NIVE.filtroInvitacion; }, '__ALL__');
    if (iv && iv !== '__ALL__') f.push(iv === 'SI' ? 'con invitación' : 'sin invitación');
    /* FASE 3.3 · tanda B (20/08/2026) */
    var of = leer(function () { return NIVE.filtroOferta; }, '__ALL__');
    if (of && of !== '__ALL__') f.push(of === 'SI' ? 'con la oferta pagada' : 'sin pago de la oferta');
    if (t) f.push('búsqueda "' + t + '"');
    return f;
  }
  /* Los que ya tienen puntaje, como números. */
  function nivelPuntajes(L) {
    return L.filter(function (r) { return r.puntaje !== '' && r.puntaje !== null && r.puntaje !== undefined; })
            .map(function (r) { return Number(r.puntaje); })
            .filter(function (n) { return isFinite(n); });
  }
  function nivelProm(ns) {
    if (!ns.length) return 0;
    return ns.reduce(function (s, n) { return s + n; }, 0) / ns.length;
  }
  function nivelMediana(ns) {
    if (!ns.length) return 0;
    var o = ns.slice().sort(function (a, b) { return a - b; });
    var m = Math.floor(o.length / 2);
    return o.length % 2 ? o[m] : (o[m - 1] + o[m]) / 2;
  }
  function dec(n) { return (Math.round(n * 100) / 100).toFixed(2); }
  /* Días transcurridos desde un sello "dd/mm/yyyy - h:mm AM/PM". */
  function nivelDias(sello) {
    var m = String(sello || '').match(/^(\d{2})\/(\d{2})\/(\d{4})/);
    if (!m) return '';
    var d = new Date(+m[3], +m[2] - 1, +m[1]);
    if (isNaN(d.getTime())) return '';
    var hoy = new Date();
    var dias = Math.floor((new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate()) - d) / 86400000);
    return (dias >= 0 && dias < 4000) ? dias : '';
  }
  ...
  var PANELES = {
    ...
    'view-nivel': {
      ...
      botones: [
        ...
        { id: 'niveles',    et: 'Por nivel de inglés',     ic: '🗣️' },
        { id: 'promedio',   et: 'Promedio del grupo',      ic: '📊' },
        { id: 'pendientes', et: 'Sin puntaje todavía',     ic: '⏳' },
        { id: 'listos',     et: 'Listos para ofertas',     ic: '🇺🇸' },
        { id: 'condicion',  et: 'Deben hacer el curso',    ic: '📚' },
        { id: 'repetir',    et: 'Tienen que repetir',      ic: '🔁' },
        { id: 'corte',      et: 'Qué tan cerca del corte', ic: '🎯' },
        { id: 'envios',     et: 'Mensajes enviados',       ic: '📨' },
        { id: 'formulario', et: 'Avance del formulario',   ic: '📝' },
        { id: 'invitacion', et: 'Invitaciones',            ic: '✉️' },
        { id: 'oferta',     et: 'Pago de la oferta',       ic: '💵' },
        { id: 'contacto',   et: 'Datos de contacto',       ic: '📇' },
        { id: 'espera',     et: 'Cuánto llevan esperando', ic: '⏱️' },
        { id: 'recientes',  et: 'Últimos que entraron',    ic: '🆕' },
        { id: 'siguiente',  et: 'Qué hago ahora',          ic: '✅' }
      ]
    }
  };
  ...
  /* ============================================================
     INFORMES — NIVEL DE INGLÉS (Fase 3 SEP · Entrega 2)
     ============================================================ */
  function infNivel(id) {
    var L = nivel();
    ...
    if (id === 'resumen') {
      ...
      return {
        ...
        texto: 'Tienes ' + b(T) + ' ' + plural(T, 'estudiante', 'estudiantes') + ' en pantalla.\n\n' +
          ...
          '- Perfil aprobado: ' + b(apro) + '\n' +
          (ns.length ? '- Puntaje promedio: **' + dec(nivelProm(ns)) + '** / 9 (más alto ' + dec(Math.max.apply(null, ns)) +
                       ', más bajo ' + dec(Math.min.apply(null, ns)) + ')\n' : '') +
          '\nEl resultado que más pesa es **' + est[0].k + '** con ' + b(est[0].n) + ' (' + pct(est[0].n, T) + ' %).' + cola,
        voz: 'Tienes ' + T + ' estudiantes en pantalla. ' + CON.length + ' ya tienen puntaje y ' + SIN.length +
             ' no. ' + (ns.length ? 'El promedio es ' + dec(nivelProm(ns)) + ' sobre 9.' : '')
      };
    }
    ...
    if (id === 'resultado') {
      var c = contar(L, function (r) { return etiq(r.estadoLabel); });
      return {
        titulo: '🏷️ Por resultado',
        texto: 'Así se reparten los ' + b(T) + ' ' + plural(T, 'estudiante', 'estudiantes') + ':\n\n' + ranking(c, T, 8) + cola,
        voz: 'Por resultado: ' + c.slice(0, 4).map(function (x) { return x.k + ', ' + x.n; }).join('; ') + '.'
      };
    }

    /* AJUSTE 3 (19/08/2026) — INVITACIONES. Solo cuenta la marca de la
       tarjeta: no manda ni mide mensajes. */
    if (id === 'invitacion') {
      var conInv = L.filter(function (r) { return !!r.invitacion; });
      var sinInv = L.filter(function (r) { return !r.invitacion; });
      if (!conInv.length) {
        return {
          titulo: '✉️ Invitaciones',
          texto: 'De los ' + b(T) + ' ' + plural(T, 'estudiante', 'estudiantes') + ' en pantalla, ' +
                 '**ninguno** tiene la invitación marcada todavía.\n\n' +
                 'La marca se pone con el botón ✉️ de cada tarjeta y no envía ningún mensaje: ' +
                 'solo deja constancia de a quién ya invitaste.' + cola,
          voz: 'Todavía no hay invitaciones marcadas.'
        };
      }
      var quien = contar(conInv, function (r) { return etiq(r.invitacionPor); });
      var listado = conInv.slice(0, 15).map(function (r) {
        return '- ' + r.nombres + ' ' + r.apellidos +
               (r.invitacionFecha ? ' · ' + r.invitacionFecha : '') +
               (r.invitacionPor ? ' · ' + r.invitacionPor : '');
      }).join('\n');
      return {
        titulo: '✉️ Invitaciones',
        texto: 'Tienes ' + b(conInv.length) + ' ' + plural(conInv.length, 'invitación marcada', 'invitaciones marcadas') +
               ' (' + pct(conInv.length, T) + ' %) y ' + b(sinInv.length) + ' sin invitar.\n\n' +
               listado + (conInv.length > 15 ? '\n\n…y ' + (conInv.length - 15) + ' más.' : '') +
               '\n\nQuien más ha invitado es **' + quien[0].k + '** con ' + b(quien[0].n) + '.' + cola,
        voz: 'Hay ' + conInv.length + ' invitaciones marcadas y ' + sinInv.length + ' estudiantes sin invitar.'
      };
    }

    /* FASE 3.3 · tanda B (20/08/2026) — PAGO DE LA OFERTA. Se cuenta
       lo mismo que muestra la tarjeta: pagada es el check MÁS el
       comprobante (lo que de verdad le abre el formulario), y aparte
       se avisa de los que tienen el check sin soporte, que es el caso
       que deja al estudiante trancado sin que se note. */
    if (id === 'oferta') {
      var pag = L.filter(function (r) { return !!r.pagoOferta; });
      var falta = L.filter(function (r) { return !r.pagoOferta && r.pagoOfertaCheck && !r.pagoOfertaComprobante; });
      var sin = L.filter(function (r) { return !r.pagoOferta && !r.pagoOfertaCheck; });
      var colaFalta = falta.length
        ? '\n\n⚠️ ' + b(falta.length) + ' ' + plural(falta.length, 'tiene', 'tienen') +
          ' el check del pago **sin comprobante**: hasta que no se suba, ' +
          plural(falta.length, 'ese estudiante no puede', 'esos estudiantes no pueden') +
          ' entrar al formulario.\n' +
          falta.slice(0, 10).map(function (r) { return '- ' + r.nombres + ' ' + r.apellidos; }).join('\n')
        : '';
      if (!pag.length) {
        return {
          titulo: '💵 Pago de la oferta',
          texto: 'De los ' + b(T) + ' ' + plural(T, 'estudiante', 'estudiantes') + ' en pantalla, ' +
                 '**ninguno** tiene todavía el pago de la oferta validado.\n\n' +
                 'El pago se marca en la vista **Contador**, en el bloque 2: check "Pago oferta OK" ' +
                 'más su comprobante. Con los dos puestos se le envía el acceso al formulario.' +
                 colaFalta + cola,
          voz: 'Todavía nadie tiene el pago de la oferta validado.'
        };
      }
      var listaPago = pag.slice(0, 15).map(function (r) {
        return '- ' + r.nombres + ' ' + r.apellidos + (r.pagoOfertaFecha ? ' · ' + r.pagoOfertaFecha : '');
      }).join('\n');
      return {
        titulo: '💵 Pago de la oferta',
        texto: 'Ya pagaron la oferta ' + b(pag.length) + ' de ' + b(T) + ' (' + pct(pag.length, T) + ' %) ' +
               'y ' + b(sin.length) + ' ' + plural(sin.length, 'sigue', 'siguen') + ' sin pagar.\n\n' +
               listaPago + (pag.length > 15 ? '\n\n…y ' + (pag.length - 15) + ' más.' : '') +
               colaFalta + cola,
        voz: 'Hay ' + pag.length + ' estudiantes con la oferta pagada y ' + sin.length + ' sin pagar.' +
             (falta.length ? ' Ojo: ' + falta.length + ' tienen el check sin comprobante.' : '')
      };
    }

    if (id === 'niveles') {
      if (!CON.length) return { titulo: '🗣️ Por nivel de inglés', texto: 'Todavía nadie tiene Puntaje SEA registrado, así que no hay niveles que repartir.' + cola, voz: 'Todavía nadie tiene puntaje.' };
      var cn = contar(CON, function (r) { return etiq(r.nivel); });
      return {
        titulo: '🗣️ Por nivel de inglés',
        texto: 'De los ' + b(CON.length) + ' que ya presentaron:\n\n' + ranking(cn, CON.length, 12) +
          '\n\nEl nivel más común es **' + cn[0].k + '**.' + cola,
        voz: 'El nivel más común es ' + cn[0].k + ', con ' + cn[0].n + ' estudiantes.'
      };
    }

    if (id === 'promedio') {
      if (!ns.length) return { titulo: '📊 Promedio del grupo', texto: 'Nadie tiene puntaje todavía, así que no hay promedio que sacar.' + cola, voz: 'Nadie tiene puntaje todavía.' };
      var prom = nivelProm(ns), med = nivelMediana(ns);
      var arriba = ns.filter(function (n) { return n >= 5.5; }).length;
      var medio  = ns.filter(function (n) { return n >= 4.5 && n < 5.5; }).length;
      var abajo  = ns.filter(function (n) { return n < 4.5; }).length;
      return {
        titulo: '📊 Promedio del grupo',
        texto: 'Sobre ' + b(ns.length) + ' ' + plural(ns.length, 'puntaje', 'puntajes') + ' registrados:\n\n' +
          '- Promedio: **' + dec(prom) + '** / 9\n' +
          '- Mitad del grupo (mediana): **' + dec(med) + '**\n' +
          '- Más alto: **' + dec(Math.max.apply(null, ns)) + '** · más bajo: **' + dec(Math.min.apply(null, ns)) + '**\n\n' +
          'Repartidos por el corte:\n' +
          '- 5.50 o más (aceptados): ' + b(arriba) + ' (' + pct(arriba, ns.length) + ' %)\n' +
          '- entre 4.50 y 5.49 (con condición): ' + b(medio) + ' (' + pct(medio, ns.length) + ' %)\n' +
          '- menos de 4.50 (no aceptados): ' + b(abajo) + ' (' + pct(abajo, ns.length) + ' %)' + cola,
        voz: 'El promedio del grupo es ' + dec(prom) + ' sobre 9. ' + arriba + ' pasaron de 5 punto 5 y ' + abajo + ' quedaron por debajo de 4 punto 5.'
      };
    }

    if (id === 'pendientes') {
      if (!SIN.length) return { titulo: '⏳ Sin puntaje todavía', texto: 'Todos los que ves ya tienen su Puntaje SEA registrado. 🎉' + cola, voz: 'No hay nadie sin puntaje.' };
      return {
        titulo: '⏳ Sin puntaje todavía',
        texto: 'Faltan por registrar ' + b(SIN.length) + ' de ' + b(T) + ' (' + pct(SIN.length, T) + ' %):\n\n' +
          SIN.slice(0, 15).map(function (r) {
            return '- ' + r.nombres + ' ' + r.apellidos + ' · N° ' + r.n + (r.whatsapp ? ' · 📱 ' + r.whatsapp : '');
          }).join('\n') +
          (SIN.length > 15 ? '\n- _y ' + (SIN.length - 15) + ' más_' : '') + cola,
        voz: 'Faltan ' + SIN.length + ' estudiantes por registrar su puntaje.'
      };
    }

    if (id === 'listos') {
      var listos = L.filter(function (r) { return r.estado === 'ACEPTADO' || r.estado === 'APROBADO'; });
      if (!listos.length) return { titulo: '🇺🇸 Listos para ofertas', texto: 'Todavía nadie alcanzó el nivel de aceptado directo con los filtros puestos.' + cola, voz: 'Todavía nadie está aceptado.' };
      var pn = nivelPuntajes(listos);
      return {
        titulo: '🇺🇸 Listos para ofertas',
        texto: b(listos.length) + ' de ' + b(T) + ' (' + pct(listos.length, T) + ' %) aprobaron sin condición' +
          (pn.length ? ', con un promedio de **' + dec(nivelProm(pn)) + '**' : '') + ':\n\n' +
          listos.slice(0, 15).map(function (r) {
            return '- ' + r.nombres + ' ' + r.apellidos + ' · ' + dec(Number(r.puntaje)) + ' · ' + r.nivel;
          }).join('\n') +
          (listos.length > 15 ? '\n- _y ' + (listos.length - 15) + ' más_' : '') +
          '\n\nNo necesitan el curso de inglés ni presentar otra vez el SET.' + cola,
        voz: listos.length + ' estudiantes están aceptados y pueden avanzar a las ofertas.'
      };
    }

    if (id === 'condicion') {
      var cond = L.filter(function (r) { return r.estado === 'ACEPTADO_CONDICION'; });
      if (!cond.length) return { titulo: '📚 Deben hacer el curso', texto: 'Nadie quedó aceptado con condición con los filtros puestos.' + cola, voz: 'Nadie quedó con condición.' };
      return {
        titulo: '📚 Deben hacer el curso',
        texto: b(cond.length) + ' ' + plural(cond.length, 'estudiante', 'estudiantes') + ' (' + pct(cond.length, T) + ' %) ' +
          plural(cond.length, 'pasó', 'pasaron') + ' con requisito de preparación: tienen que completar el Curso de Inglés de SEP Colombia antes de escoger oferta.\n\n' +
          cond.slice(0, 15).map(function (r) {
            return '- ' + r.nombres + ' ' + r.apellidos + ' · ' + dec(Number(r.puntaje)) + ' · ' + r.nivel;
          }).join('\n') +
          (cond.length > 15 ? '\n- _y ' + (cond.length - 15) + ' más_' : '') + cola,
        voz: cond.length + ' estudiantes deben completar el curso de inglés antes de escoger oferta.'
      };
    }

    if (id === 'repetir') {
      var no = L.filter(function (r) { return r.estado === 'NO_ACEPTADO'; });
      if (!no.length) return { titulo: '🔁 Tienen que repetir', texto: 'Ninguno quedó por debajo del mínimo con los filtros puestos. 🎉' + cola, voz: 'Nadie quedó por debajo del mínimo.' };
      var pn2 = nivelPuntajes(no);
      var cerca = no.filter(function (r) { return Number(r.puntaje) >= 4; }).length;
      return {
        titulo: '🔁 Tienen que repetir',
        texto: b(no.length) + ' de ' + b(T) + ' (' + pct(no.length, T) + ' %) no alcanzaron el mínimo' +
          (pn2.length ? ', con un promedio de **' + dec(nivelProm(pn2)) + '**' : '') + '.\n' +
          'De esos, ' + b(cerca) + ' ' + plural(cerca, 'está', 'están') + ' entre 4.00 y 4.49: les faltó poco.\n\n' +
          no.slice(0, 15).map(function (r) {
            return '- ' + r.nombres + ' ' + r.apellidos + ' · ' + dec(Number(r.puntaje)) + ' · ' + r.nivel;
          }).join('\n') +
          (no.length > 15 ? '\n- _y ' + (no.length - 15) + ' más_' : '') +
          '\n\nPueden volver a presentar el SET dentro de 30 días.' + cola,
        voz: no.length + ' estudiantes deben repetir la prueba, y a ' + cerca + ' les faltó muy poco.'
      };
    }

    if (id === 'corte') {
      if (!ns.length) return { titulo: '🎯 Qué tan cerca del corte', texto: 'Nadie tiene puntaje todavía, así que no hay a quién medirle la distancia al corte.' + cola, voz: 'Nadie tiene puntaje todavía.' };
      var casi   = L.filter(function (r) { return r.puntaje !== '' && Number(r.puntaje) >= 4.2 && Number(r.puntaje) < 4.5; });
      var justo  = L.filter(function (r) { return r.puntaje !== '' && Number(r.puntaje) >= 4.5 && Number(r.puntaje) < 4.8; });
      var casi55 = L.filter(function (r) { return r.puntaje !== '' && Number(r.puntaje) >= 5.2 && Number(r.puntaje) < 5.5; });
      return {
        titulo: '🎯 Qué tan cerca del corte',
        texto: 'Los dos cortes que deciden todo son **4.50** (entrar con condición) y **5.50** (entrar sin condición).\n\n' +
          '- A menos de 0.30 de entrar (4.20 a 4.49): ' + b(casi.length) + '\n' +
          '- Entraron raspando (4.50 a 4.79): ' + b(justo.length) + '\n' +
          '- A menos de 0.30 de librarse del curso (5.20 a 5.49): ' + b(casi55.length) + '\n\n' +
          (casi.length ? 'Los que quedaron a un pelo:\n' + casi.slice(0, 10).map(function (r) {
            return '- ' + r.nombres + ' ' + r.apellidos + ' · ' + dec(Number(r.puntaje));
          }).join('\n') : 'Nadie quedó pegado al corte por debajo.') + cola,
        voz: casi.length + ' estudiantes quedaron a menos de tres décimas de entrar, y ' + casi55.length + ' a menos de tres décimas de librarse del curso.'
      };
    }

    if (id === 'envios') {
      var totEnv = L.reduce(function (s, r) { return s + (Number(r.envios) || 0); }, 0);
      var rep = L.filter(function (r) { return (Number(r.envios) || 0) > 1; });
      return {
        titulo: '📨 Mensajes enviados',
        texto: 'Se han enviado ' + b(totEnv) + ' ' + plural(totEnv, 'mensaje', 'mensajes') + ' de resultado a ' +
          b(CON.length) + ' ' + plural(CON.length, 'estudiante', 'estudiantes') + ' (WhatsApp y correo).\n\n' +
          '- Con un solo envío: ' + b(CON.length - rep.length) + '\n' +
          '- Con el puntaje corregido y reenviado: ' + b(rep.length) + '\n\n' +
          (rep.length ? 'Los reenviados:\n' + rep.slice(0, 10).map(function (r) {
            return '- ' + r.nombres + ' ' + r.apellidos + ' · ' + r.envios + ' envíos · ' + dec(Number(r.puntaje));
          }).join('\n') : 'Ningún puntaje se ha tenido que corregir.') + cola,
        voz: 'Se han enviado ' + totEnv + ' mensajes de resultado, y ' + rep.length + ' estudiantes recibieron más de uno.'
      };
    }

    if (id === 'formulario') {
      var cf = contar(L, function (r) { return etiq(r.formularioEstado); });
      var completos = L.filter(function (r) { return /completo/i.test(String(r.formularioEstado || '')); }).length;
      return {
        titulo: '📝 Avance del formulario',
        texto: 'Cómo va el formulario de la Zona de estudiantes:\n\n' + ranking(cf, T, 8) +
          '\n\nCompletos: ' + b(completos) + ' de ' + b(T) + ' (' + pct(completos, T) + ' %).' +
          '\n\n_El formulario se construye en las Entregas 3 y 4: hasta entonces todos figuran igual._' + cola,
        voz: 'Del formulario hay ' + completos + ' completos de ' + T + '.'
      };
    }

    if (id === 'contacto') {
      var sinWa = L.filter(function (r) { return !txt(r.whatsapp); });
      var sinMail = L.filter(function (r) { return !txt(r.correo); });
      var sinDoc = L.filter(function (r) { return !txt(r.documento); });
      var sinDir = L.filter(function (r) { return !txt(r.direccion); });
      var sinNac = L.filter(function (r) { return !txt(r.nacimiento); });
      var sinClave = L.filter(function (r) { return !txt(r.claveAcceso); });
      var mudos = L.filter(function (r) { return !txt(r.whatsapp) && !txt(r.correo); });
      return {
        titulo: '📇 Datos de contacto',
        texto: 'De los ' + b(T) + ' en pantalla, esto es lo que falta:\n\n' +
          '- Sin WhatsApp: ' + b(sinWa.length) + '\n' +
          '- Sin correo: ' + b(sinMail.length) + '\n' +
          '- Sin documento: ' + b(sinDoc.length) + '\n' +
          '- Sin dirección: ' + b(sinDir.length) + '\n' +
          '- Sin fecha de nacimiento: ' + b(sinNac.length) + '\n' +
          '- Sin clave del portal: ' + b(sinClave.length) + '\n\n' +
          (mudos.length
            ? '⚠️ **' + mudos.length + '** no tienen ni WhatsApp ni correo: a esos el resultado NO les llega.\n' +
              mudos.slice(0, 8).map(function (r) { return '- ' + r.nombres + ' ' + r.apellidos + ' · N° ' + r.n; }).join('\n')
            : 'Todos tienen al menos un canal para recibir el resultado. ✅') + cola,
        voz: mudos.length ? mudos.length + ' estudiantes no tienen ni WhatsApp ni correo.' : 'Todos tienen al menos un canal de contacto.'
      };
    }

    if (id === 'espera') {
      var conF = L.map(function (r) { return { r: r, d: nivelDias(r.fechaMigracion) }; })
                  .filter(function (x) { return x.d !== ''; });
      if (!conF.length) return { titulo: '⏱️ Cuánto llevan esperando', texto: 'No hay fechas de migración legibles en pantalla.' + cola, voz: 'No hay fechas de migración.' };
      var esperando = conF.filter(function (x) { return x.r.estado === 'SIN_PUNTAJE'; })
                          .sort(function (a, b2) { return b2.d - a.d; });
      var viejos = esperando.filter(function (x) { return x.d >= 7; });
      var promD = conF.reduce(function (s2, x) { return s2 + x.d; }, 0) / conF.length;
      return {
        titulo: '⏱️ Cuánto llevan esperando',
        texto: 'Desde que se les validó el contrato y entraron a este proceso:\n\n' +
          '- Llevan en el proceso, en promedio, **' + (Math.round(promD * 10) / 10) + '** ' + plural(Math.round(promD), 'día', 'días') + '\n' +
          '- Sin puntaje todavía: ' + b(esperando.length) + '\n' +
          '- De esos, con más de una semana esperando: ' + b(viejos.length) + '\n\n' +
          (esperando.length
            ? 'Los que llevan más tiempo sin resultado:\n' + esperando.slice(0, 10).map(function (x) {
                return '- ' + x.r.nombres + ' ' + x.r.apellidos + ' · ' + x.d + ' ' + plural(x.d, 'día', 'días') +
                       (x.r.fechaMigracion ? ' (desde ' + x.r.fechaMigracion + ')' : '');
              }).join('\n')
            : 'Nadie está esperando resultado. 🎉') + cola,
        voz: esperando.length
          ? esperando.length + ' estudiantes esperan resultado, y ' + viejos.length + ' llevan más de una semana.'
          : 'Nadie está esperando resultado.'
      };
    }

    if (id === 'recientes') {
      var ult = L.slice().sort(function (a, b2) { return b2.n - a.n; }).slice(0, 12);
      return {
        titulo: '🆕 Últimos que entraron',
        texto: 'Los últimos ' + b(ult.length) + ' que pasaron a este proceso (entran al validarse su contrato):\n\n' +
          ult.map(function (r) {
            return '- N° ' + r.n + ' · ' + r.nombres + ' ' + r.apellidos +
                   (r.fechaMigracion ? ' · ' + r.fechaMigracion : '') +
                   ' · ' + r.estadoLabel;
          }).join('\n') + cola,
        voz: 'El último en entrar fue ' + ult[0].nombres + ' ' + ult[0].apellidos + '.'
      };
    }

    if (id === 'siguiente') {
      var tareas = [];
      var mudos2 = L.filter(function (r) { return !txt(r.whatsapp) && !txt(r.correo); }).length;
      if (SIN.length) tareas.push('Registrar el Puntaje SEA de ' + b(SIN.length) + ' ' + plural(SIN.length, 'estudiante', 'estudiantes') + ' que todavía no lo tienen.');
      if (mudos2) tareas.push('Completar el contacto de ' + b(mudos2) + ' que no tienen ni WhatsApp ni correo: a ellos el resultado no les llega.');
      var cond2 = L.filter(function (r) { return r.estado === 'ACEPTADO_CONDICION'; }).length;
      if (cond2) tareas.push('Hacerle seguimiento al curso de inglés de ' + b(cond2) + ' ' + plural(cond2, 'aceptado con condición', 'aceptados con condición') + '.');
      var no2 = L.filter(function (r) { return r.estado === 'NO_ACEPTADO'; }).length;
      if (no2) tareas.push('Ofrecerle Direct English a ' + b(no2) + ' ' + plural(no2, 'estudiante', 'estudiantes') + ' que deben repetir el SET en 30 días.');
      var listos2 = L.filter(function (r) { return r.estado === 'ACEPTADO'; }).length;
      if (listos2) tareas.push('Revisar el formulario de ' + b(listos2) + ' ' + plural(listos2, 'aceptado', 'aceptados') + ' para poder aprobar su perfil.');
      return {
        titulo: '✅ Qué hago ahora',
        texto: (tareas.length
          ? 'Con lo que tienes en pantalla, por orden:\n\n' + tareas.map(function (t, i) { return (i + 1) + '. ' + t; }).join('\n')
          : 'No hay nada pendiente con los filtros que tienes puestos. 🎉') + cola,
        voz: tareas.length ? 'Lo primero: ' + String(tareas[0]).replace(/\*\*/g, '') : 'No hay pendientes con los filtros puestos.'
      };
    }
  ...
  function informe(vista, id) {
    ...
    if (vista === 'view-nivel') return infNivel(id);
  ...
  window.__sep11 = {
    ...
    comercial: comercial, contador: contador, tablero: tablero, nivel: nivel,
